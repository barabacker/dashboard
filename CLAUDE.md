...
src/
  ...
  control/              everything authored/stored + admin/dashboard surface. ALL models here.
    models.py             Collector projection, Config, Source (table, referenced by Config), Schedule, Job
    services/             the single shared enqueue function
    forms/                authoring forms: Config (a field per ParamSpec of the chosen collector) and Source (a field per site attribute)
...
| D16 | ~~The tender-site parsers are **vendored** into `collectors/engine/`, framework-free. A *collector* is a parser family (`tender_fogsoft`, `tender_kendo`, `tender_btorg`, `tender_ruson`); a *site* is authored data — domain, listing path and TLS quirks are ordinary parameters. No `platforms.toml`.~~ **Superseded by D22** — a site's domain/listing path/TLS quirks are no longer a Config parameter at all; they live on `control.models.Source`, a real table. The vendoring and "collector = parser family" framing stand unchanged. | See ADR 0002 for the original reasoning, ADR 0004 for why a `Config`-parameter site stopped being enough once one site needed several named collection profiles. |
| D17 | ~~The «Источники» tab is `control.models.Source` (renamed from `Platform` on 2026-07-30 — the manager still only shows tender trading-platform sites, but the label had to stop implying every future source will be one), a **proxy of Config** with its own form, not a table.~~ **Superseded by D22** — `Source` stops being a `Config` proxy; it becomes its own table, and `Config` references it via an optional FK. | The rename reasoning (the label must not imply every future source is a trading platform) still holds. What no longer holds is "no table of its own" — see ADR 0004 for why the proxy stopped being enough. |
...
| D22 | **`control.models.Source` stops being a `Config` proxy and becomes its own table** (`name`, `domain`, `listing_path`, `extra_ca_cert`, `skip_tls_verify`, `archived`), referenced by an optional `Config.source` FK (`null=True`, `on_delete=PROTECT`). `collector_key` stays on `Config`, not duplicated onto `Source` — one `Source`, many named `Config` profiles (`default`/`full`/`fast`), each with its own `name` and behavioural parameters. `effective_parameters` at enqueue merges `Source` fields (filtered to the ones the target collector's schema declares) with `Config.parameters` — the two key sets are disjoint by construction, so there is no override semantics to define. `listing_path` stays authored on `Source` with a code default (`DEFAULT_LISTING_PATHS[engine]`), not hardcoded — the existing per-site exceptions (`sistematorg`, `promkonsalt`) need the escape hatch. `Source.save()` cascades a `revision` bump onto its Configs via a direct `update(revision=F("revision") + 1)`, the same pattern `record_job_outcome`/`forget_job_outcomes` already use. `CollectorDescriptor.is_site: bool = False` (schema-level, no DB) marks which collectors are site-shaped; `ConfigForm.clean()` rejects a site-shaped `collector_key` with no `source` and a `source` on a collector that declares no site parameters, both with a named error on the `source` field rather than a generic parameter complaint or a silent no-op. Not a queryset filter — `Source` is no longer a filtered subset of `Config` rows, so `SourceManager` is removed entirely. Full narrative: ADR 0004. | Two forcing factors arrived together: one site needing several named collection profiles that must not duplicate domain/TLS by hand, and a genuinely different collector family (a real-estate site) exposing that `SourceForm`'s hardcoded field union never generalized past the four near-identical tender engines. An earlier draft of this decision introduced a third concept (`Site`) alongside `Source`/`Config`; rejected as an unneeded new term — `Source` already meant "a site, authored" everywhere it was used, so it is `Source` itself that stops being a proxy, not a new entity bolted on beside it. Snapshot completeness is untouched — `effective_parameters` is still resolved once at enqueue into a flat dict; this only changes where the *authored* data before that resolution lives. Supersedes **D16** and **D17** together. |
| D23 | **`ConfigForm` renders one field per `ParamSpec`** of the selected `collector_key` (`str/int/float/bool/choices → CharField/IntegerField/FloatField/BooleanField/ChoiceField`, `list`/`dict → JSONField`), replacing the old raw-JSON `parameters` textarea. Fields also declared on `Source` (`Source.PARAM_FIELDS`) are skipped — supplied by the profile's `source`, not re-asked. Only one collector's fields render at a time: the `collector_key` dropdown reloads the add page via `?collector_key=`, no JS framework, so field names never collide across collectors. `ConfigAdmin.get_form()`/`get_fieldsets()` are both overridden — `get_fieldsets()` builds a throwaway `self.form(...)` instance (not `self.get_form(...)`, which would recurse back into `get_fieldsets()`) to read the current parameter field names; `get_form()` calls `modelform_factory` directly with no `fields=` override, since Django's default passes `flatten_fieldsets(get_fieldsets(...))` into it and those dynamic parameter names are not real `Config` model fields, which `modelform_factory` rejects. `ConfigInline` (D22) gets its own `ConfigInlineForm(ConfigForm)` subclass overriding `_add_parameter_fields` to a no-op, so the inline's deliberately-narrow three-column view doesn't grow extra fields. Full narrative: ADR 0005. | Closes the deferred half of the question ADR 0004 opened: `SourceForm` turned out not to need dynamic rendering (its fields became plain model fields), but `Config`'s own behavioural parameters (`max_pages`/`only_active`/`concurrency`/`fetch_details`) still did — a hand-maintained field list would need editing for every future collector, exactly what D22 was set up to avoid for sources. A review of whether any of the four should move to code instead found none should: `fetch_details` is the `full`/`fast` axis the profile system exists for, `only_active` is a genuine operational choice, `max_pages` is a debugging knob by its own schema description, and `concurrency` — despite no carried-over site ever using a non-default value — is a working, tested engine capability (`test_kendo_crawl_is_correct_under_higher_concurrency`), not a vestigial one. |
