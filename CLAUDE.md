...
src/
  ...
  control/              everything authored/stored + admin/dashboard surface. ALL models here.
    ...
    forms/                authoring forms: raw JSON, or a per-collector form built from ParamSpec
                           for any collector flagged CollectorDescriptor.is_source (D22)
...
| D22 | **Source decoupled from `tender_*` and its own proxy (supersedes D17).** `control.models.Source` is deleted; `Config` is the only model and the only registered admin. `CollectorDescriptor` gains `is_source: bool` (default `False`); `ParamSpec` gains `choices_provider` for a runtime-computed pick list. `ConfigAdmin` dispatches per request: a collector with `is_source=True` gets a form built field-by-field from its `ParamSpec`s (`control/forms/source.py::SourceForm`); anything else gets the raw-JSON `ConfigForm`. Adding "add" — a collector must be picked before its fields can render — is two-step: `add/` with no `?collector_key=` shows a plain picker, submitting it is a GET to `add/?collector_key=<key>`, which then renders the right form. Migration `0007` (`DeleteModel(Source)` + relabelling `Config` to «Источник»/«Источники»). | The tender-only hardcoding (`SourceForm.PARAM_FIELDS` as a hand-maintained union of all four engines' parameters, `SourceManager` filtering `collector_key__startswith="tender_"`) stopped scaling the moment a second, materially different collector family (e.g. a future real-estate site) was on the table — every new family would have meant editing the shared form and the manager's filter by hand. `is_source` lives on `CollectorDescriptor`, never on the `Collector` DB projection, for the same reason D14 keeps schema/version off that row: a second place to store the same fact about a collector drifts from the first. No new table (D17's underlying reasoning still holds, only its "separate proxy + tab" conclusion is retired), no domain-uniqueness constraint added — `Config` still allows several rows per site (e.g. named "full"/"default" presets for the same domain), which this change leaves untouched on purpose. |
