...
src/
  ...
  control/              everything authored/stored + admin/dashboard surface. ALL models here.
    models.py             Collector projection, Config, Source (proxy), Site, Schedule, Job
...
| D22 | **`Site` is a new table** (`control.models.Site`: `name`, `domain`, `listing_path`, `extra_ca_cert`, `skip_tls_verify`, `archived`), referenced by an optional `Config.site` FK (`null=True`, `on_delete=PROTECT`). `collector_key` stays on `Config`, not duplicated onto `Site`. `effective_parameters` at enqueue merges `Site` fields (filtered to the ones the target collector's schema declares) with `Config.parameters` — the two key sets are disjoint by construction, so there is no override semantics to define. `listing_path` stays authored on `Site` with a code default (`DEFAULT_LISTING_PATHS[engine]`), not hardcoded — the existing per-site exceptions (`sistematorg`, `promkonsalt`) need the escape hatch. `Site.save()` cascades a `revision` bump onto its Configs via a direct `update(revision=F("revision") + 1)`, the same pattern `record_job_outcome`/`forget_job_outcomes` already use. `CollectorDescriptor.is_site: bool = False` (schema-level, no DB) replaces the `tender_` prefix string `SourceManager` used to filter on. `SourceForm` renders fields per the selected collector's `ParamSpec` list instead of a hardcoded union. Full narrative: ADR 0004. | Two forcing factors arrived together: one site needing several named collection profiles (`default`/`full`/`fast`) that must not duplicate domain/TLS by hand, and a genuinely different collector family (a real-estate site) exposing that `SourceForm`'s hardcoded field union never generalized past the four near-identical tender engines. Snapshot completeness is untouched — `effective_parameters` is still resolved once at enqueue into a flat dict; this only changes where the *authored* data before that resolution lives. Supersedes the parameter half of **D16**; narrows **D17** (still a proxy, no table of its own for the *profile* — but no longer the only new state below «Источники»). |
